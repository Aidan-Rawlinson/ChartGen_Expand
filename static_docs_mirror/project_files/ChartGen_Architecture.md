...
| `core/output_generation/execution/charts/chart_store_xlsx.py` | Excel export/import round-trip for `chart_store.csv` (via `CG_Extracts`, Decision 42) — full-replace, mirroring `stat_tags_xlsx.py`'s own pattern. See Decision 28 |
...
| `core/output_generation/execution/tables/base_tables/` | Four built-in Base Tables (`plain_grid`, `table_cardtile`, `ci_grid`, `ci_cardtile`) -- trimmed back from ten to two (Decision 30), with `ci_grid`/`ci_cardtile` added afterward as CI-report-specific two-row-header variants (Decision 50) -- one standalone file per `table_type_ref`, no shared helpers module -- the same self-containment convention as `base_charts/` (Decision 18); dispatch in `registry.py`. Returns `(image_bytes, chart_cells)`, not bare bytes. See Decisions 24, 29, 30, and 50 |
...
| `workfile_config/running_order.csv` | ★ Canonical Running Order store — flat table, not `.xlsx`. The `.xlsx` is generated from this on demand for export and parsed back into it on import (via `CG_Extracts`, Decision 42); it is never itself written to this archive |
...
**Excel round-trip is full-replace, not identity-merge.** `stat_tags_xlsx.py` mirrors the Running Order xlsx's own simple pattern (export the current rows, edit, import replaces the whole list) rather than the manifest table's `hex_id`-keyed add/update/soft-delete round trip — a Stat Tag has no cached data of its own to preserve behind a "deleted" flag, so a row absent from the imported file is simply gone, matching the Text tab's own Delete button. A row read back with a blank `tag` is issued a fresh one (`assign_missing_tags`) rather than left blank, since a blank tag could never be matched to anything anyway.
...
**Font: Calibri.** `matplotlib.rcParams["font.family"] = "Calibri"` is set in every Base Chart/Base Table file. `matplotlib.rcParams["svg.fonttype"]` was originally left at its own default, `"path"` (text baked into vector outline shapes, not real `<text>` elements) — **superseded, see Decision 48**, which replaces this with real `<text>` (`"none"`) everywhere, via a draw-big-then-shrink workaround for the PowerPoint compression artefact that made `"none"` unworkable on its own.

**Known limitations, accepted.**
- Table/chart text is not searchable or selectable in PowerPoint or the exported PDF — this held under the original `"path"` approach; whether it still holds under Decision 48's `"none"` approach hasn't been re-verified (Decision 48 was scoped to the spacing/corruption problem specifically, not this one).
...
**"Show Chart Store" replaces the preview, never sits beside it.** Toggling it switches the Charts sheet's whole right-hand content area over to a table of every Chart Store entry — mirroring Stat Tags' own table (Decision 19): read-only, one row per entry, delete/export/import only (Decision 42), no inline grid editing. The rest of the left rail (Select Visualisation, Populations, Tweaks, Sizing, both Save controls, Custom Charts, Zoom, Reset) doesn't render at all while the table is shown, since none of it means anything without a chart actually being previewed.

**Excel round-trip mirrors Stat Tags, not Output Tables.** A Chart Store entry is a flat row, like a Stat Tag — not a grid, like an Output Table — so `chart_store_xlsx.py` mirrors `stat_tags_xlsx.py`'s full-replace export/import pattern (export current rows, edit, import replaces the whole list; a row re-imported with a blank `chart_store_id` is issued a fresh one) rather than `grid_xlsx.py`'s own spreadsheet-shaped one.
...
### Decision 31 — Chart Store Population Inheritance for Table-Cell Rendering

A Chart Store entry's own blank `populations` field is meant to inherit the Running Order's current default — the same rule any `insert_chart` row's own blank `populations` field already follows — but the two chart-in-table-cell rendering paths (`insert_table.py`'s final-report path, and the Output Tables tab's own Preview splice, both Decision 30) read the entry's `populations` field directly, bypassing that inherit rule entirely. An entry saved relying on inheritance (e.g. to pick up "Selected" from the workfile default) silently lost that layer specifically when rendered inside a table cell, while rendering correctly everywhere else the same entry is used.

**Fixed at both call sites, by different means, since only one has the mechanism the other needs.** `insert_table.py` has an `AssemblyContext` in scope, so it now reads `ctx.default_populations` directly — the same live value `insert_chart` itself already reads, set by a `set_default_populations` Running Order row earlier in the same run. The Preview splice has no `AssemblyContext` (it runs from the Streamlit tab, not a report assembly run), so it instead reads the workfile's own `set_default_populations` row directly off `running_order_rows` — mirroring the Charts sheet's own Preview, which already had this same fallback for its own (non-table) chart preview.

### Decision 32 — Chart Store Id Issuance Made Self-Healing

`next_chart_store_id` (Decision 28) issued ids from a persisted counter (`settings["next_chart_store_id"]`) with no check against ids already present on `chart_store_rows`. If the counter fell behind what was actually in use — confirmed: a row carried in via Excel import with its own `chart_store_id` already filled in never advances the counter, since `assign_missing_chart_store_ids` only issues a fresh id for a genuinely blank cell — a subsequently-issued id could collide with one already sitting on another row, a real duplicate rather than a hypothetical one.

**Fixed by resyncing before issuing, not by trusting the stored counter alone.** `next_chart_store_id` now accepts an optional `existing_ids` set; when given, the counter is first advanced to the true maximum found among them (decoded via `id_generation.from_base36`, the new inverse of the existing `to_base36`) before being incremented — a new id is always "the current maximum across everything, plus one," never issued from the stored counter in isolation. Both entry points now supply this: the Charts sheet's "Add new entry" button passes every id currently in `chart_store_rows`; `chart_store_xlsx.py`'s own import path (`assign_missing_chart_store_ids`) seeds the same set from every non-blank id already in the imported rows, updating it as each new id is issued so two blank rows in the same import can't collide with each other either.

### Decision 33 — Line Breaks in Output Table Content (`<br>`)

Output Table cell content had no way to represent an intentional line break. A literal newline can't be typed into a cell via the Output Tables tab's own grid editor (a single-line text-cell editor, the same limitation `st.data_editor` has generically); and even where a real newline character did reach a Base Table's own rendering step by some other route, nothing accounted for the extra vertical space it needs — font-fitting was width-only, so a multi-line cell's true rendered height was never checked against its own row, risking the overflow being silently painted over by the row drawn immediately after it, rather than visibly clipped or otherwise apparent.

**A typed `<br>` (also `<br/>`, `<br />`, case-insensitive) is now converted to a real newline in `resolve.py`** — the one point both the final report (`insert_table.py`) and the Output Tables Preview already resolve content through, applied after Stat Tag substitution. A Base Table function itself never needs to know the `<br>` convention exists; it only ever needs to handle a genuine embedded `"\n"`, which matplotlib's own text rendering already supports natively (rendered as stacked lines).

**Any Base Table style intending to support this correctly needs its own height-aware font-fit step**, not just the width-based fit every style already had — checking a multi-line cell's actual rendered height against its own row's real available height, and shrinking the font further if it wouldn't otherwise fit, is what keeps the additional line(s) genuinely visible rather than merely present in the file.

### Decision 34 — Excel Percentage Values Read via Number Format, Not Raw Cell Value

`grid_xlsx.py`'s `read_output_table_xlsx` read `cell.value` directly for every cell. Excel's own behaviour: typing `5%` into a cell auto-applies a Percentage number format and stores the underlying value as the raw float `0.05` — the `%` is purely display formatting, never part of the value itself. Reading `cell.value` alone therefore silently returned the literal text `"0.05"` as table content, discarding the percentage representation entirely, wherever a person had typed a percentage directly into a cell.

**Fixed by consulting `cell.number_format` alongside the value, not instead of it.** A new `_cell_text` helper checks whether `cell.number_format` contains `"%"`; when it does, the underlying float is multiplied back up by 100 and returned with a `"%"` appended (rounded first, to avoid a result like `"5.000000000000001%"`), matching what Excel itself actually displays rather than the raw stored number. Ordinary text and plain (non-percentage-formatted) numbers are unaffected.

**The same class of gap — reading `cell.value` without checking `cell.number_format` — hasn't been checked for in the other Excel round-trip readers** (Stat Tags, Chart Store, the Running Order's own xlsx pair). Only the Output Table content path was reported and is confirmed fixed here; the others are a live, un-investigated risk of the same failure mode, not yet ruled out.

### Decision 35 — Optional Chart Detail in the Custom Tables Bundle

A Custom Tables download bundle (Decision 24) previously covered the table's own code and grid only — a `{Cn}` chart-component cell (Decision 30) was opaque to it, since the table function itself never touches the chart it names. `build_bundle` gained an opt-in `include_charts` flag (a checkbox in the Output Tables tab's own Custom Tables section, off by default): when set, the bundle scans `content` for every distinct chart-component marker and, for each one still resolvable in `chart_store_rows`, appends that entry's own settings, complete rendering source, and live `population_layers` — the same three things a stand-alone Custom Charts bundle (Decision 18) would show — plus one explanatory section up front describing the two-step render-then-composite relationship, so the whole table (embedded charts included) can be rebuilt from this one document alone. Off by default since resolving each chart's live data has a real cost, and most table edits don't touch what's inside one.
...
### Decision 42 — Excel Round-Trips Moved from Browser Widgets to `CG_Extracts`

Every Excel export/import round-trip (chart URL manifest, Running Order, Chart Store, Stat Tags, Output Table grids, population tables) previously went through `st.download_button`/`st.file_uploader` — both browser-sandboxed widgets, with no way for server-side code to choose where a download lands or to default an upload dialog to a particular folder. Every one now writes directly to, and reads directly from, a fixed `CG_Extracts` folder created alongside the `.cgw` (`core/shared/infrastructure/cg_extracts.get_extracts_folder`), replacing the browser's own Downloads folder as the destination/source.

Export is a plain `st.button` that writes the file straight to disk (no dialog) and confirms via `st.success` with the full path. Import is a plain `st.button` that opens a native `tkinter` Open dialog (`core.ui.common.pickers.pick_xlsx_file`), defaulted to `CG_Extracts` via `initialdir` but not restricted to it — the same native-dialog mechanism `pick_workfile_file`/`pick_save_file` already used for `.cgw` Open/Save, extended to `.xlsx`. Every `write_*_xlsx`/`read_*_xlsx` function already accepted a plain path rather than requiring a buffer, so no changes were needed to any reader/writer itself — only to the UI layer around them.

Filenames are unchanged (`chart_store.xlsx`, `running_order.xlsx`, `stat_tags.xlsx`, `{table_id}_grid.xlsx`, `{table_name}.xlsx`, `{workfile_name}_chart_urls.xlsx`); they now all sit together in one shared folder rather than the browser's Downloads folder. Reachable only from screens that require an already-saved `.cgw`, so no unsaved-workfile fallback exists.

**Out of scope.** The Custom Charts/Custom Tables bundle downloads (`.md`, code hand-off to an AI) are unaffected — they aren't an Excel round-trip and stay on `st.download_button`.

### Decision 43 — PairedSurveyData: A Fifth Canonical Data Shape

The four canonical data shapes (Decision-era design, Functional Spec Section 8.2) all hold one value, one composition, or one time-indexed value per unit per metric. None of them fit a dataset where each unit contributes a *collection of individual records* — the Sunderland/Modified Barthel chart's per-patient start/end score pairs, mapped from `sp_a_generic_survey_sunderland_score_chart`, needed exactly this: patient identity has no meaning beyond distinguishing rows (a plain positional label, "Patient 1" — confirmed not identifiable or pseudonymised patient data before this was accepted as a real field), and the shape's stats need pooling across every record across every unit, not averaging pre-computed per-unit stats.

**New module** `core/shared/normalisation_containers/shapes/paired_survey_data.py`: `PairedObservation` (`patient_label`, `start_value`, `end_value`), `PairedSurveyDataUnit(Unit)` (a `records` list, in place of `values`/`response`), `PairedSurveyDataStats` (`count_with_data` — a record counts if it has at least one of start/end present — `count_null`, `mean_start`, `mean_end`; deliberately minimal, no median/quartiles/min/max), `PairedSurveyData` itself, `compute_paired_survey_data_stats`, `paired_survey_data_summary_stats`, `filter_paired_survey_data`.

**Structural choice: flat `units` list, not a `metrics` list.** NumericCompositional and CategoricalCompositional wrap their per-unit data in a `metrics` list to support more than one Metric-Series per shape instance. PairedSurveyData is confirmed always exactly one Metric-Series — not designed to extend — so it follows NumericSeries' flatter pattern instead: a single shape-level `units` list, no metrics wrapper.

**Wired into the three generic dispatch points** (`filter_shape`, `summary_stats`, `shape_units` in `dispatch.py`) and into `unit_has_data` (checks `records` for any record with a present start or end value, alongside the existing `values`/`response` checks). Registered in `shapes/__init__.py`'s exports. `cache_reader.py` gained `_from_dict_paired_survey_data` and a `DESERIALISE_MAP` entry for cache read support; cache *write* needed no change, since `cache_writer.save_chart` already serialises any shape generically via `dataclasses.asdict`.
...
### Decision 48 — SVG Text Rendering Corrected: Real `<text>`, via a Draw-Big-Then-Shrink Workaround

Supersedes the "Font: Calibri, baked into vector outlines, not live text" paragraph of Decision 27, and the corresponding rule in every Base Chart/Base Table file and the `custom_charts`/`custom_tables` contracts. Documents the current, replacement mechanism in full — the underlying problem, why Decision 27's original approach doesn't work, and the fix now in place everywhere.

**The problem Decision 27 didn't solve.** With `svg.fonttype` left at its own default (`"path"`, text baked into vector outline shapes), text is genuinely searchable/selectable nowhere — the accepted cost Decision 27 recorded. Switching to `"none"` (real `<text>` elements) was tried and reverted at that time for a different reason: PowerPoint's own SVG handling applies a lossy compression pass over an embedded SVG's content, and when text is kept as real `<text>` rather than pre-baked outline paths, that pass measurably mis-spaces individual characters — most visibly on decimal-heavy labels ("0.000" rendering with mismatched gaps between the zeros). This is what actually produced the "characters selectable in mismatched positions" finding Decision 27 recorded — a compression artefact, not a searchability limitation, and not specific to any one Base Table style (a hypothesis raised and confirmed wrong this session).

**The fix: draw everything bigger, shrink it back down on placement.** Every Base Chart and Base Table is now called with `width_emu`/`height_emu` multiplied by a fixed factor (`5`, currently) before it draws anything, then the resulting image is placed back onto the slide/page at the real, unmultiplied target size — `add_svg_picture` already scales an SVG's own content to whatever placement box it's given, regardless of the SVG's own native declared size, so no resize-after-insert step is needed; placement is otherwise completely unchanged from before this fix. Giving PowerPoint's own compression pass more absolute pixel/point precision to work with, then shrinking the whole result back down uniformly, resolves the mis-spacing with no visible cost.

This alone is only half the mechanism. A bigger canvas alone does nothing for a chart's own **absolute** point-based sizes — font sizes, line widths, marker sizes, dash-pattern lengths, fixed-inch padding — since none of those scale automatically just because the canvas grew. Left unscaled, text/lines would render correctly proportioned to the *design* but far too small once the whole oversized image is shrunk back down to real size. **Every individual Base Chart/Base Table therefore also carries its own local `TEXT_SCALE` constant**, multiplying every absolute literal of this kind in that file. This is the one genuinely fragile part of the whole mechanism: `TEXT_SCALE` in every Base Chart/Base Table file, and `CHART_RENDER_SCALE` in every system-layer call site (`assembly_engine.py`, `insert_table.py`, `charts_tab.py`, `output_tables_tab.py`), must all carry the identical value. Nothing enforces this in code — Base Charts/Tables remain standalone artefacts with no shared import (Decision 18/24's own framing) — a mismatch anywhere silently produces incorrectly-proportioned text/lines in that one file only, not a visible error.

**Not every literal needs touching.** Some Base Charts already compute their own sizes proportionally from the figure's own physical dimensions (e.g. the `line_ci_*` family of small circular status-marker charts derive linewidth/fontsize from a radius that's itself a fraction of `width_emu`/`height_emu`) — these scale correctly automatically and need only the `svg.fonttype` change, no `TEXT_SCALE` constant. A fixed physical constant (an absolute inch offset, a fixed font-size search bound) always needs scaling; a fraction of the axes/figure (0–1 data-space or figure-fraction coordinates) never does.

**`svg.fonttype` is now `"none"` everywhere**, in every Base Chart and Base Table, replacing the earlier system-wide `"path"` default from Decision 27 — no exceptions; this is a full replacement of the earlier rendering methodology, not a per-file opt-in.

**Chart-in-table-cell placement (Decision 30) needed a further, distinct correction.** A Base Table called with the inflated size reports its `chart_cells` rectangle (the reserved area for an embedded Chart Store chart) in that same inflated render-space, not real placement space, since the rectangle is derived proportionally from whatever `width_emu`/`height_emu` the table itself was actually given. The embedded chart's own render call uses that rectangle's raw width/height directly (already correctly inflated, matching what a Base Chart expects under this mechanism) — but the *placement* of the resulting picture on the slide (`insert_table.py`'s own `add_svg_picture` call) must divide that same rectangle by the scale factor first, to convert it back to a real slide-EMU offset relative to the table's own real position. Both `insert_table.py` and the Output Tables tab's own Preview splice were updated to divide correctly at the placement boundary only, never at the render boundary.

**Custom Charts/Custom Tables contracts (Decisions 18, 24) updated to match.** `width_emu`/`height_emu` handed to a custom chart/table are now genuinely the inflated value, not the real target size — the contract documents explicitly tell an AI author this, and require the same `svg.fonttype`/`TEXT_SCALE` treatment in any custom code, mirroring every built-in. A custom chart/table saved before this change needs re-saving with the new pattern applied by hand — the same class of breaking change Decision 27 itself was.

**Export Picture and both sandbox previews render the same, genuinely oversized artefact as the final report does.** Export Picture's standalone `.svg` file is deliberately the oversized version (matching exactly what's embedded in a PPTX), not shrunk back down for standalone viewing; both the Charts sheet and Output Tables tab previews render at the same inflated size and display at the real size via CSS, the same shrink mechanism PowerPoint itself performs.

**Factor chosen: `5`.** An earlier exploratory pass (fully reverted, no trace left in the codebase) proved the same mechanism at `4`. This session's initial working version used `10` before settling on `5` as the better balance against render time — a bigger factor costs real matplotlib render time per chart (roughly proportional to canvas area, so quadratic in the factor), with diminishing returns against the underlying PowerPoint compression artefact once past a moderate multiple.

### Decision 49 — PDF Export: Guaranteed PowerPoint Cleanup and a Save-Settle Guard

Two related but independently-discovered `save_pdf` (Decision 26's module) reliability fixes, both from the same session as Decision 48, addressing PowerPoint being left open/lingering after a run.

**Guaranteed cleanup, success or failure.** `deck.Close()`/`powerpoint.Quit()` previously ran only on the success path — any exception during `ExportAsFixedFormat`, `Presentations.Open()`, or `deck.Close()` itself skipped both, leaving a visible, orphaned `POWERPNT.EXE` process indefinitely, with only an error message in the log to show for it. Nested `try`/`finally` blocks now guarantee each cleanup step runs regardless of what fails above it — `deck.Close()` always runs once `Presentations.Open()` succeeds, `powerpoint.Quit()` always runs once `CreateObject` succeeds, independent of each other's success/failure. No change to what gets reported: the same outer `except` still returns the same `err_result` for a genuine failure.

**A distinct, second cause of lingering PowerPoint — reference-counting, not exceptions.** Confirmed separately: PowerPoint could still fail to close even on a fully successful run (clean export, correct PDF, no error anywhere in the log) — `comtypes` can hold a COM interface pointer alive slightly longer than the Python variable's own scope suggests, so `Quit()` genuinely being called and succeeding doesn't guarantee the underlying process actually unloads. Fixed with explicit `del deck` / `del powerpoint` immediately after `Quit()`, dropping both references to zero so CPython's ordinary reference counting (not garbage collection) releases them immediately. A full `gc.collect()` was tried first and confirmed to work, but at a real cost — 10-30 seconds observed, since a full collection walks the entire live object graph, sized by everything a report run leaves in memory (every chart's own matplotlib figures/SVG buffers, larger again since Decision 48's scaling work) — and was removed once the plain `del`s were confirmed sufficient on their own. If PowerPoint is ever observed lingering again after this, `gc.collect(0)` (youngest generation only, far cheaper than a full collection) is the next thing to try before reaching for a full `collect()` again — that would point to a genuine reference cycle, not scope-exit timing.

**A separate, unrelated failure mode: a modal dialog blocking automation against a SharePoint/OneDrive-synced output path.** Observed once: `save_pdf` failed with `"Cannot perform this action with a modal dialog showing"` after PowerPoint sat open for a long time, on a workfile whose output folder syncs via OneDrive/SharePoint (Decision 8's own stated deployment target) — the same `.pptx` opened cleanly by hand afterwards, ruling out actual corruption. Suspected cause: COM's `Presentations.Open()` catching the file mid-write or mid-sync-lock, not a genuine content problem.

**Fixed with a shared save-and-settle helper, not a longer timeout or a silent retry.** `_delete_existing_and_save` (`assembly_engine.py`), used by both `save_ppt` and `save_pdf`: deletes any existing file at the target path first (confirming the deletion actually completed — this is always an overwrite of a previous run's output, never a first save), saves, then checks the file's size across two checks 0.3 seconds apart, up to 5 attempts (roughly 1.5 seconds worst case) before proceeding — negligible from a human point of view in the normal case, enough to avoid handing a file that's still being flushed/synced straight to COM automation. Raises loudly if deletion or settling doesn't complete within its own bounded attempts, rather than retrying indefinitely or proceeding with an unstable file — consistent with Section 2's "Validate only where designed" principle: this is a deliberate, designed check at a known-risky boundary, not a defensive guard layered in ad hoc.

### Decision 50 — `ci_grid`/`ci_cardtile`: Two Further Built-In Base Tables, Undocumented Until Now

`registry.py`'s own docstring, and this document's Section 4 file tree and Decision 24/30's own counts, all stated "two built-in Base Tables" (`plain_grid`, `table_cardtile`) — stale. The actual `TABLE_REGISTRY` has carried four for some time: `plain_grid`, `table_cardtile`, `ci_grid`, `ci_cardtile` — the latter two being two-row-header, CI-report-specific variants of `plain_grid`/`table_cardtile` respectively (two merged header rows, fixed "Benchmark"/"Target"/"Met(?)" column labels, blank-row suppression for a fixed row count against a variable real row count). Both received Decision 48's own `svg.fonttype`/`TEXT_SCALE` treatment this session, alongside every other Base Chart/Base Table.

This gap is corrected here rather than silently: it caused a real mistake this session (a report using `ci_cardtile` was initially misdiagnosed as using an unrelated custom table, since the file wasn't expected to exist under that name in the built-in registry going into the investigation). `registry.py`'s own docstring and this document's Section 4 file tree are updated to list all four.
