...
| `insert_chart` | Renders a Base Chart from cached data; inserts at the row's stored position. Position and size come from the Running Order row, populated automatically from the template. Optionally draws a hyperlink icon after the chart, positioned relative to the chart's own top-right corner and linked to the chart's own data shape's `metadata["source_url"]` — see Architecture, Decisions 37–38. |
...
*No Base Chart function currently reads its `tweaks` parameter.* The column, the parameter, and the round-trip through the Charts sheet all work end to end; no chart's rendering varies with the value yet. See Architecture, Decision 16.
