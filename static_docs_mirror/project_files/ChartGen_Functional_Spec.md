...
This is why the actual code for a single visualisation — a **Base Chart** — sits outside ChartGen's own core process rather than inside it. A Base Chart is a self-contained rendering function, not application logic: it depends on nothing else in the codebase, and nothing else in the codebase depends on its internals. It receives **chart_inputs** — a fixed four-parameter call: `population_layers` (an ordered list of filtered data-shape copies, one per resolved population token — Section 10.4), `width`/`height` (percent of the shorter page dimension — Section 10.6), and `tweaks` (below) — and returns image bytes; what happens in between is entirely its own concern.
...
**Grid.** An Output Table's content is authored as an (N+1) × (M+1) grid: row 0 and column 0 hold column widths, row heights, and the table's own id (corner cell, display only); the remainder is content cells — constant text, or a Stat Tag id (`[3]`). Column widths and row heights are each independently expected to sum to ~100% of the table's own width/height, validated on an explicit Update (tolerance ±0.5%). New tables default to equal widths/heights (`100/count`). Grid content, once authored, is available for both Preview rendering and the underlying `.cgw`.
...
**Base Tables.** Ten built-in `table_type_ref`s, each a standalone rendering function taking **table_inputs** — `content` (resolved grid), `column_widths`, `row_heights`, `width`, `height`, `tweaks` — and returning image bytes only, the table equivalent of `chart_inputs` (Section 10.0). No shape-type scoping: every Base Table takes the same resolved grid, so any one is a valid choice for any Output Table.

| table_type_ref | Description |
|---|---|
| `plain_grid` | Plain white cells, black borders, centred text |
| `table_ledger` | Ledger/accounting style — thin rules, serif figures, right-aligned values |
| `table_zebra` | Zebra-striped banded rows, solid header band |
| `table_editorial` | Editorial rule-based style (FT/Economist influence) — no vertical rules |
| `table_terminal` | Terminal/monospace dark-mode style |
| `table_cardtile` | Card-tile metrics — rounded white cards with a soft shadow, right-aligned values |
| `table_pill` | Pill/badge cells — rounded label tabs, coloured value badges |
| `table_freeform` | Borderless, free-form minimalist style |
| `table_brutalist` | Neo-brutalist — thick black rules, hard corners, flat colour header block |
| `table_softui` | Soft-UI gradient dashboard — gradient header band, rounded translucent value badges |
...
**Running Order.** `insert_table` (Section 9.2) renders and inserts an Output Table the same way `insert_chart` does a chart — position and size from the Running Order row; `table_type_ref` resolved built-in-then-custom.

**Known gaps.** Chart-component cells (`{n}`, referencing a saved chart embedded in a cell) are recognised by the grid's own grammar but not resolved or rendered — see Feature List.
...
- **Stat Tags** (e.g. `[3]`, `[a7]`) — short, permanent, system-issued ids, each standing in for one summary-stats value from one chart's own independently-authored cut of its cached data. See Section 12.1.
...
**Tag ids** are base-36 (`0`–`9` then `a`–`z`), issued from a persisted, monotonically increasing counter, never reused even after a tag is deleted.
