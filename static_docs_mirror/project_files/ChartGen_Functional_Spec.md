...
Not built. A Base Chart function's job is producing a visual; summary statistics and unit lists are read from the data shapes directly, on demand, by whatever needs them (Section 9.4) — nothing currently computes or stores chart statistics ahead of a consumer needing them. Autotables, when built, would call the same shape-level statistics functions directly against the relevant `insert_chart` row's data shape and resolved population layers, rather than reading anything relayed through the Chart Engine. See Architecture, Decision 17.
