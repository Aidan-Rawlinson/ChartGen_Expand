...
- **Units included** — one table per population layer, listing every unit's id, code, and name. Name is resolved from the population table the chart's data belongs to, since a shape's own unit records carry only id and code.
