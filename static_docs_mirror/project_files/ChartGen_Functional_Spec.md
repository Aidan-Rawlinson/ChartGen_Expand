...
An `.xlsx` version can be generated by the system as a human-editing format for the Running Order — created on demand for download and parsed back in on upload. It is never written to disk on its own and never persisted inside the `.cgw` itself. Dropdown validation constrains `function`, `enabled`, `scope`, and `base_chart_name` (per-row, filtered to valid chart types for the assigned cache file's data shape, including this workfile's own saved Custom Charts for that shape — Section 10.9 — also accounting for `metric_periods`, see Section 10.8) on each export, plus `start_period`, `end_period`, and `metric_periods` (per-row, against that cache file's own period list via a hidden list sheet — see Architecture Decision 12). It is editable directly in Excel and uploadable/downloadable via the Streamlit Running Order tab.
...
**Edit Grid** is content authoring: the raw grid (column widths, row heights, content cells), a resize control, an explicit Update (validating column-width/row-height sums, see Section 11.2), and an Excel Download/Upload round-trip — a full-replace pattern mirroring the grid's own spreadsheet shape, with a Stat Tag id dropdown for content cells via a hidden list sheet, free text still accepted alongside it.
...
**Storage and editing.** One row per tag in `workfile_config/text_stats.csv` (Architecture Section 5). An Excel download/upload round-trip is available: the whole list is replaced on upload (a row absent from the uploaded file is deleted, matching the Text tab's own Delete button) — the same full-replace pattern the Running Order's own xlsx uses, not the manifest table's identity-merge one.
