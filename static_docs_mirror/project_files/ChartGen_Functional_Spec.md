...
- **Table** — the text matches `[Table:name,Rows:X,Columns:Y]` exactly (whitespace tolerated around `[`, `,`, `:`, `]`, otherwise this precise structure only). Associates the box with an Output Table by name — the same name reuses that Output Table's existing grid and current content unchanged on re-upload; `Rows`/`Columns` are only applied when the name is new. See Section 11.2.
...
An `.xlsx` version can be generated by the system as a human-editing format for the Running Order — created on demand for download and parsed back in on upload. It is never written to disk on its own and never persisted inside the `.cgw` itself. Dropdown validation constrains `function`, `enabled`, `scope`, and `chart_type_ref` (per-row, filtered to valid chart types for the assigned cache file's data shape, including this workfile's own saved Custom Charts for that shape — Section 10.9 — also accounting for `metric_periods`, see Section 10.8) on each export, plus `start_period`, `end_period`, and `metric_periods` (per-row, against that cache file's own period list via a hidden list sheet — see Architecture Decision 12). It is editable directly in Excel and uploadable/downloadable via the Streamlit Running Order tab.
...
| `insert_table` | Renders a Base Table from an Output Table's grid; inserts at the row's stored position, the same way `insert_chart` does. `table_type_ref` resolves built-in-then-custom, the same way `chart_type_ref` does for Custom Charts. See Section 11.2. |
...
**Creation.** Either directly on the Output Tables tab ("+ New Output Table" — name, rows, columns), or via a `[Table:name,Rows:X,Columns:Y]` yellow box (Section 6.3) at template processing. Re-uploading a template with a `[Table:...]` box whose name matches an existing Output Table reuses that table's `table_id` and leaves its grid untouched; `Rows`/`Columns` only apply when the name is new.
...
**Known gaps.** Chart-component cells (`{n}`, referencing a saved chart embedded in a cell) are recognised by the grid's own grammar but not resolved or rendered — see Feature List. A manually-created Output Table (not from a yellow box) has no route onto the Running Order except via the Output Tables tab's own Save to Running Order (Insert above/below an existing row) — there is no template-driven placement for it.
