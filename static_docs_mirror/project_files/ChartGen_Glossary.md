...
- **`CG_Chart_`/`CG_Link_` shape naming** — the naming `insert_chart` gives the two PowerPoint shapes it creates: the chart picture itself (`CG_Chart_{row_id}`) and its optional hyperlink icon (`CG_Link_{row_id}`), keyed on the Running Order row's own `row_id` at the point of insertion. Exists for traceback, and so the Position Finder tool (below) can match a hyperlink icon back to its own chart by name. Accepted to go stale once the Running Order is next reordered or edited, since `row_id` itself renumbers then — a deliberately simpler scheme than a genuinely stable id. See Architecture, Decision 40.
...
- **Position Finder** — a read-only Running Order support tool (not a Running Order function itself) that reads a selected shape's live position/size off an already-open PowerPoint via COM, for copying into a Running Order row by hand. Resolves a `CG_Link_` icon's position as an offset from its matching `CG_Chart_` shape (see above) where one is found on the same slide. See Functional Spec, Section 9.7, and Architecture, Decision 41.
...
- **Stat tag** — a short, permanent, system-issued, `T`-prefixed id (base-36, e.g. `[T3]`, `[Ta7]`) standing in for one summary-stats value from one chart's own independently-authored cut of its cached data — a single population token, optional TimeSeries period range/metric-periods conversion, and a Reference id (see Cluster 10). Defined and previewed on the Text tab; resolved by `update_text` at generation time, in ordinary text and PowerPoint table cells alike. Not tied to any Running Order row. Distinct from Text Tag (below) — a Reference id isn't globally unique, so a Stat Tag needs hex id + population + reference id to mean anything, where a Text Tag needs nothing beyond its own literal string. See Functional Spec, Section 12.1, and Architecture, Decisions 19 and 30.
...
- **chart_inputs** — the fixed four-parameter call every Base Chart function receives: `population_layers`, `width_emu`, `height_emu`, `tweaks`. See Functional Spec, Section 10.0, and Architecture, Decision 29.
...
- **Chart Store** — a flat, unordered set of independently-authored chart-defs (`workfile_config/chart_store.csv`), used as chart components inside Output Table cells (Cluster 12) — independent of the Running Order, which is strictly a sequence of report content. Each entry carries the same fields as `CHART_SANDBOX_FIELDS` plus a permanent, never-reused, `C`-prefixed `chart_store_id` (base-36, via the shared `id_generation` counter) and an optional description. Authored from the Charts sheet as a third entry point ("Chart Store line") alongside Running Order row and Data shape; Add/Overwrite only on save-back, no position concept. See Functional Spec, Section 10.10, and Architecture, Decisions 28 and 30.
...
- **Tweak** — the `tweaks` component of the chart_inputs contract (see chart_inputs) — a free-text Running Order column, uninterpreted by anything outside the Base Chart function it's passed to. Two Base Charts (`column_ci_full`, `line_ci_full`) currently read it, each via its own `key:value^key2:value2` caret-delimited convention — a de facto pattern, not one ChartGen enforces on any other chart. See Architecture, Decision 16.
...
- **Base Table** — one of ChartGen's table-rendering functions, each a standalone artefact, the table equivalent of a Base Chart. No shape-type scoping — every Base Table takes the same table_inputs. Two built-in styles (`plain_grid`, `table_cardtile`), trimmed back from ten. See Architecture, Decisions 24 and 30.

- **table_inputs** — the fixed six-parameter call every Base Table function receives: `content`, `column_widths`, `row_heights`, `width_emu`, `height_emu`, `tweaks`; returns `(image_bytes, chart_cells)`. The table equivalent of chart_inputs (Cluster 10). See Functional Spec, Section 11.2, and Architecture, Decisions 29 and 30.

- **Chart-component cell** — a grid cell holding a Chart Store marker (`{Cn}`), naming a chart that belongs in that cell. Not drawn as text — the Base Table function reports back the cell's own rectangle (in EMU) instead, via `chart_cells`; something else renders the named Chart Store entry into that rectangle, sized to fit it rather than the entry's own stored size. Rendered as a layered PowerPoint picture in the final report, or a spliced-in SVG `<image>` in the Output Tables Preview. See Architecture, Decision 30.
