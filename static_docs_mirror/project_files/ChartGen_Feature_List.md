...
| Population tables — Excel export/import round-trip | Partial | Per-table export/import on the Populations tab (via `CG_Extracts`, Architecture Decision 42), generic across any table. Identity is `unit_id`: unmatched id added, missing id removed (no soft-delete flag on these tables), blank id skipped. No validation of edited values — a removed `unit_id` can leave a dangling `soft_parents` reference elsewhere. See Functional Spec Section 7.2. |
...
| Direct URL entry (Excel round-trip) | Complete | Export formatted `.xlsx` to `CG_Extracts`, add rows with just a URL, import back. Row deletion removes the chart from the table; cached data and identity are retained. |
...
| Running Order .xlsx for user entry with formatting and validation (export/import) | Complete | Via `CG_Extracts` — Architecture Decision 42. |
...
| Feature | Readiness | Notes |
|---|---|---|
| Base Chart library (20 charts across 4 data shapes with chart coverage) | Complete | No chart type renders a title. Each is a standalone artefact with no shared internal helpers — see Custom Charts, below. Takes `width_emu`/`height_emu` directly (Architecture Decision 29). Renders as SVG with Calibri, real `<text>` via a draw-big-then-shrink workaround (Architecture Decisions 27, 48). |
| PairedSurveyData (fifth canonical data shape) | Partial | Shape built and wired into dispatch/cache-read (Architecture Decision 43) — can be constructed, filtered, and have its stats read. No transformer populates one from real API data yet, no Base Chart accepts it, and no Summary Stat Tags integration exists. |
...
| Output Table Excel export/import round-trip | Complete | Full-replace, via `CG_Extracts` (Architecture Decision 42), mirroring the grid's own spreadsheet shape directly (not a flat table) — content cells offer a Stat Tag id dropdown via a hidden list sheet, free text still accepted alongside it. A percentage-formatted cell (e.g. typed as `5%`) is read back via its own number format, not its raw underlying value. See Architecture Decision 34. |
| Base Table library (4 styles) | Complete | `plain_grid`, `table_cardtile`, and their two-row-header CI-report variants `ci_grid`, `ci_cardtile` — trimmed back from ten to two (Architecture Decision 30), with `ci_grid`/`ci_cardtile` added afterward (Architecture Decision 50). Each a standalone artefact, no shared internal helpers, taking `table_inputs` (resolved grid, sizing, tweaks) and returning `(image_bytes, chart_cells)` — the second value reports any chart-component cell's own rectangle, in EMU (see below). See Architecture Decisions 24, 29, 30, 50. Renders as SVG with Calibri, real `<text>` via a draw-big-then-shrink workaround (Architecture Decisions 27, 48). |
