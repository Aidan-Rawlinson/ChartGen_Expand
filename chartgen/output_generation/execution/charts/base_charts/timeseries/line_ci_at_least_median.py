"""Base Chart, TimeSeries, diagnostic. Three-way circle indicator on the Selected unit's final-period value only. The circle is always blue; the white mark inside carries the result: a tick if the value is at least the scope's own median for that period, a cross if not, a dash if either is missing."""
