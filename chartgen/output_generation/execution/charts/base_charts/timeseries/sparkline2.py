"""Base Chart, TimeSeries, sparkline variant. Composite widget: a "has
data" indicator (dot + "count/12" months-with-data label) sits
immediately before the sparkline; the sparkline always shows a fixed
12-month window, working backwards from the most recent period (older
months are dropped if there are more than 12, and the window is padded
with "no data" months at the start if there are fewer than 12). The
sparkline draws two lines, the scope's median per period and the
Selected unit's own value per period. Any period where the median is
missing is filled in with a flat, dotted grey line (and matching grey
shading) held at the nearest known median height, so the median line
always spans the full width even with gaps; the submission line is left
exactly as before (nothing extra is drawn if it has no data at all).
Finishing (right-hand) values for both series are labelled in their own
reserved space so they can never be clipped off. After the sparkline,
two small trend badges show whether the submission and the median
increased, decreased, stayed the same, or can't be compared (not
applicable) between the first and last period of the 12-month window.
No axes, ticks, gridlines, legend or title, so it renders cleanly at
very small sizes. Peer-group layers are ignored."""
...
# the canvas, so nothing drawn right at the start (the has-data circle's
# own outline) risks being cropped by the canvas boundary itself.
...
# Has-data indicator (dot + "count/12" label), drawn immediately before
# the sparkline. The text box is sized tightly to "12/12" - the longest
# this label can ever be - rather than a generous guess, so there's no
# dead space between the number and the start of the sparkline.
HASDATA_CIRCLE_RADIUS_IN = 1.4 * MM_IN_INCHES * TEXT_SCALE
...
TREND_BADGE_GAP_IN = 6.0 * MM_IN_INCHES * TEXT_SCALE
TREND_SECTION_WIDTH_IN = 2 * CIRCLE_EDGE_PAD_IN + 4 * TREND_BADGE_RADIUS_IN + TREND_BADGE_GAP_IN
TREND_NA_FONTSIZE = 3.2 * TEXT_SCALE
TREND_LINEWIDTH = 0.6 * TEXT_SCALE

# The has-data indicator sits right up against the sparkline, so its own
# gap is much tighter than the gap used between the other sections. The
# gap before the trend badges is wider than the standard section gap, to
# put clear air between the finishing values and the first badge.
HASDATA_SPARK_GAP_IN = 0.15 * MM_IN_INCHES * TEXT_SCALE
SECTION_GAP_IN = OUTER_GAP_IN
TREND_SECTION_GAP_IN = 2.2 * MM_IN_INCHES * TEXT_SCALE
...
def _draw_gradient_fill(ax, xs, ys, baseline, top_colour, fill_zorder=0, img_zorder=0):
    if len(xs) < 2:
        return
...
def _draw_check_mark(ax, cx, cy, radius, colour="white"):
    xs = [cx - 0.50 * radius, cx - 0.08 * radius, cx + 0.55 * radius]
    ys = [cy + 0.02 * radius, cy - 0.42 * radius, cy + 0.38 * radius]
    ax.plot(xs, ys, color=colour, linewidth=0.9 * TEXT_SCALE,
...
def _draw_has_data_indicator(ax, cy, count_with_data, total):
    ...
    cx = CIRCLE_EDGE_PAD_IN + HASDATA_CIRCLE_RADIUS_IN
    circle = plt.Circle(
        (cx, cy), HASDATA_CIRCLE_RADIUS_IN,
        facecolor=colour if has_data else "none", edgecolor=colour,
        linewidth=TREND_LINEWIDTH, zorder=2,
    )
    ax.add_patch(circle)
    if has_data:
        _draw_check_mark(ax, cx, cy, HASDATA_CIRCLE_RADIUS_IN)
    ax.text(
        cx + HASDATA_CIRCLE_RADIUS_IN + HASDATA_TEXT_GAP_IN, cy,
        f"{count_with_data}/{total}", ha="left", va="center",
        fontsize=HASDATA_TEXT_FONTSIZE, fontweight="bold", color=colour, zorder=3,
    )
...
def _draw_right_labels(ax_labels, median_val, submission_val, submission_code, format_modifier):
    ...
    for (text, colour, _), y in zip(entries, y_positions):
        ax_labels.text(0.0, y, text, transform=ax_labels.transAxes, color=colour,
                        fontsize=RIGHT_LABEL_FONTSIZE, fontweight="bold",
                        ha="left", va="center")
...
def _trend(first, last):
    ...
    if f != f or l != l:  # NaN check
        return "n/a"
    if math.isclose(f, l, rel_tol=1e-9, abs_tol=1e-9):
        return "same"
...
def _draw_trend_badge(ax, cx, cy, radius, trend, series_colour):
    if trend == "n/a":
        ...
        # a glance which badge is which.
        circle = plt.Circle((cx, cy), radius, facecolor="none",
                             edgecolor=series_colour, linewidth=TREND_LINEWIDTH, zorder=2)
        ax.add_patch(circle)
        ax.text(cx, cy, "n/a", ha="center", va="center",
                fontsize=TREND_NA_FONTSIZE, fontweight="bold", color=series_colour, zorder=3)
        return
    ...
    if trend == "increased":
        ...
        bar_half_w, bar_half_h = radius * 0.6, radius * 0.16
        rect = mpatches.Rectangle((cx - bar_half_w, cy - bar_half_h),
                                   2 * bar_half_w, 2 * bar_half_h,
                                   facecolor="white", edgecolor="none", zorder=3)
        ax.add_patch(rect)


def sparkline2(population_layers: list, width_emu=2736215, height_emu=684054, tweaks=""):
    ...
    # --- Has-data indicator (dot + "count/12") -----------------------------
    ...
    # --- Sparkline ------------------------------------------------------------
    ax = fig.add_axes([
        spark_left_in / w, inner_bottom_in / h,
        spark_width_in / w, inner_height_in / h,
    ])
    ...
    if any_median_known:
        ...
        # height, shaded the same way the real (blue) line is.
        _draw_gradient_fill(ax, x, filled_medians, baseline, NEUTRAL_COL,
                             fill_zorder=0, img_zorder=0)
        ax.plot(x, filled_medians, color=NEUTRAL_COL, linewidth=1.0 * TEXT_SCALE,
                linestyle=GAP_DASH_PATTERN, dash_capstyle="round", zorder=1)
    ...
    _inset_axes_to_fit(
        fig, ax,
        spark_left_in - MARKER_OVERFLOW_MARGIN_IN, inner_bottom_in - MARKER_OVERFLOW_MARGIN_IN,
        spark_width_in + 2 * MARKER_OVERFLOW_MARGIN_IN, inner_height_in + 2 * MARKER_OVERFLOW_MARGIN_IN,
    )
