...
33 built-in Base Charts: 7 NumericSeries, 5 NumericCompositional, 6 CategoricalCompositional, 15 TimeSeries.
...
| Base Table library | Complete | Four styles: `plain_grid`, `table_cardtile`, and their two-row-header CI-report variants `ci_grid` and `ci_cardtile`. Each a standalone artefact with no shared helpers, returning image bytes plus any chart-cell rectangles. Renders as SVG with Calibri and real `<text>` |
