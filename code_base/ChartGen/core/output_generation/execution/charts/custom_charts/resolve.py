"""
resolve.py
Resolves a base_chart_name to a callable (built-in first, custom second),
...
def get_chart_callable(base_chart_name: str, custom_chart_code: dict):
    """
    ...
    if custom_chart_code and base_chart_name in custom_chart_code:
        return compile_custom_chart(custom_chart_code[base_chart_name])
...
def merge_custom_refs_for_shape(shape_type: str, built_in_refs: list, custom_chart_rows: list) -> list:
    """
    Append this workfile's saved custom base_chart_names matching shape_type
    ...
    custom_refs = [
        r["base_chart_name"] for r in custom_chart_rows
...
def custom_chart_descriptions(shape_type: str, custom_chart_rows: list) -> list:
    """
    Return (base_chart_name, description) pairs for this workfile's saved
    ...
    for r in custom_chart_rows:
        ...
        desc = f"Custom: {r['base_chart_name']} — {notes}" if notes else f"Custom: {r['base_chart_name']}"
        pairs.append((r["base_chart_name"], desc))
    return pairs
