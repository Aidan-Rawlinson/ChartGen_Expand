"""
table_brutalist.py
Base Table -- table_brutalist. Neo-brutalist style: thick black rules
around every cell, hard corners, a flat solid-colour header band. Row 0 of
...
def table_brutalist(content, column_widths, row_heights, width=80, height=50, tweaks=""):
    ...
    for r in range(n_rows):
        ...
        fc = ACCENT_YELLOW if bold else "white"
        ax.add_patch(mpatches.Rectangle((0, y0), 100, y1 - y0, facecolor=fc,
                                         edgecolor="black", linewidth=2.2))
        for c in range(n_cols):
            cx0, cx1, cy0, cy1 = _cell_bounds(p, r, c)
            val = _cell_text(p, r, c)
            if c > 0:
                ax.plot([cx0, cx0], [y0, y1], color="black", lw=2.2)
            if c == 0:
