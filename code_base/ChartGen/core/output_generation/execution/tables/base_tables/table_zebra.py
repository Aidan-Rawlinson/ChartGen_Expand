"""
table_zebra.py
Base Table -- table_zebra. Zebra-striped banded rows with a solid,
distinctly-coloured header band. Row 0 of `content` is treated as the
header row -- it is the person's own first grid row, not synthetic content
...
def table_zebra(content, column_widths, row_heights, width=80, height=50, tweaks=""):
    ...
    for r in range(n_rows):
        ...
        bold = (r == 0)
        fc = ACCENT_BLUE if bold else (GREY_PANEL if r % 2 == 0 else "white")
        ax.add_patch(mpatches.Rectangle((0, y0), 100, y1 - y0, facecolor=fc, edgecolor="none"))
