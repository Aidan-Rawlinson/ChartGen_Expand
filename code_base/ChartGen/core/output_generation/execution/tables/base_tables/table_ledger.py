"""
table_ledger.py
Base Table -- table_ledger. Ledger/accounting-style grid: thin top/bottom
rules, a double rule under the header row, serif figures, first column
left-aligned, every other column right-aligned. Row 0 of `content` is
treated as the header row (bold) -- it is the person's own first grid row,
not synthetic content injected by this function.
...
def _prepare(content, column_widths, row_heights, width, height):
    """Font-size fit and cumulative cell-boundary precompute -- shared
    groundwork this style needs, kept local rather than imported so this
    file stays a fully standalone artefact."""
...
def table_ledger(content, column_widths, row_heights, width=80, height=50, tweaks=""):
    ...
    ax.plot([0, 100], [row_y[0], row_y[0]], color="black", lw=1.1)
    if n_rows > 1:
        ax.plot([0, 100], [row_y[1], row_y[1]], color="black", lw=1.5)
        second_rule_y = row_y[1] + (row_y[-1] - row_y[0]) * 0.012
        ax.plot([0, 100], [second_rule_y, second_rule_y], color="black", lw=0.6)
    ax.plot([0, 100], [row_y[-1], row_y[-1]], color="black", lw=1.1)
        ...
        for c in range(n_cols):
            ...
            if c == 0:
                ax.text(cx0 + lp, (cy0 + cy1) / 2, val, ha="left", va="center",
                        fontsize=fs, fontweight="bold" if bold else "normal", fontfamily="serif")
            else:
