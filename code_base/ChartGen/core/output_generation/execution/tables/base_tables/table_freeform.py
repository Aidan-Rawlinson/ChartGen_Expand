"""
table_freeform.py
Base Table -- table_freeform. Borderless, free-form minimalist style: no
grid lines at all bar a single thin rule under the header row, header text
shown in muted grey. Row 0 of `content` is treated as the header row -- it
is the person's own first grid row, not synthetic content injected by this
function.
...
def table_freeform(content, column_widths, row_heights, width=80, height=50, tweaks=""):
    ...
    if n_rows > 1:
        ax.plot([0, 100], [row_y[1], row_y[1]], color=GREY_LINE, lw=0.8)
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            ...
            color = GREY_TEXT if bold else "black"
            if c == 0:
                ax.text(cx0 + lp, (cy0 + cy1) / 2, val, ha="left", va="center",
                        fontsize=fs, fontweight="bold" if bold else "normal", color=color)
            else:
