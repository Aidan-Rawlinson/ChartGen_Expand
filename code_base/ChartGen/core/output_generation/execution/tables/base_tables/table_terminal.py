"""
table_terminal.py
Base Table -- table_terminal. Terminal/monospace dark-mode style: dark
background, monospace figures, green accents for the header row and any
non-empty non-first-column value, thin divider rules between rows. Row 0
of `content` is treated as the header row -- it is the person's own first
...
def table_terminal(content, column_widths, row_heights, width=80, height=50, tweaks=""):
    ...
    ax.add_patch(mpatches.Rectangle((0, row_y[0]), 100, row_y[-1] - row_y[0],
        ...
        for c in range(n_cols):
            ...
            if c == 0:
                ...
                ax.text(cx1 - lp, (cy0 + cy1) / 2, val, ha="right", va="center",
                        ...
                        fontweight="bold" if bold else "normal", color=color)
        if r > 0:
            _, _, y0, _ = _cell_bounds(p, r, 0)
            ax.plot([0, 100], [y0, y0], color=DARK_LINE, lw=0.6)
